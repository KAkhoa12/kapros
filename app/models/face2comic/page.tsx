...
import { generateFace2Comic } from "@/lib/api";
import ReactCrop, { centerCrop, Crop, makeAspectCrop } from "react-image-crop";
import "react-image-crop/dist/ReactCrop.css";
import { useDropzone } from "react-dropzone";
import Image from "next/image";

const Face2ComicPage = () => {
  ...
  const handleGenerateImage = async () => {
    ...
    try {
      // Gọi API thực tế
      const response = await generateFace2Comic(croppedImage);
      ...
      if (response.data?.status === "success" && response.data?.data?.image) {
        ...
        setStatus({
          type: "success",
          message: response.data.message || "Tạo ảnh thành công!",
        });
      } else {
        setStatus({
          type: "error",
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="bg-gray-900 rounded-2xl p-6 border border-gray-800">
            <h2 className="text-xl font-bold text-white mb-4">Tải ảnh lên</h2>
          ...
          {/* Cột phải */}
          <div className="bg-gray-900 rounded-2xl p-6 border border-gray-800">
            <h2 className="text-xl font-bold text-white mb-4">Ảnh đã crop</h2>
            ...
            <div className="mb-6">
              <div className="bg-gray-800 rounded-lg p-4 min-h-[300px] flex items-center justify-center">
                {croppedImage ? (
                  ...
                  <p className="text-gray-500">Ảnh đã crop sẽ hiển thị ở đây</p>
                )}
              </div>
            </div>
            ...
            <div className="mt-8">
              ...
              >
                {isProcessing ? "Đang xử lý..." : "Tạo ảnh"}
              </button>
        ...
        <h2 className="text-2xl font-bold text-white m-4 text-center">Demo hướng dẫn</h2>
        <div className="grid grid-cols-3 gap-8">
            <div className="flex flex-col items-center gap-4  s">
              <Image src="/arrow_white.png" width={100} height={100} alt="" className="rounded-lg translate-x-[120px] rotate-50" />
              <Image src="/demo_1.jpg" width={256} height={300} alt="h-full w-auto" className="rounded-lg" />
              <span className="text-justify">Nhấn vào chọn ảnh của bạn</span>
            </div>
            <div className="flex flex-col items-center gap-4  justify-center">
              <Image src="/arrow_white.png" width={100} height={100} alt="" className="rounded-lg translate-x-[120px] rotate-50" />
              <Image src="/demo_2.jpg" width={256} height={300} alt="h-full w-auto" className="rounded-lg" />
              <span className="text-justify ">Crop ảnh của bạn thành tỷ lệ khung vuông 1:1 ( lưu ý crop nhớ lấy hết tất cả khuôn mặt của bạn </span>
            </div>
            <div className="flex flex-col items-center gap-4  justify-center">
              <span className="text-justify ">Generate completed!</span>
              <Image src="/demo_3.jpg" width={256} height={300} alt="h-full w-auto" className="rounded-lg" />
              <span className="text-justify ">Nhấn vào tạo ảnh và đợi kết quả!</span>
            </div>
        </div>
      </div>
